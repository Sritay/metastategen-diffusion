...
#SBATCH --qos=gpu-shd
#SBATCH --account=e760
#SBATCH --job-name=refine-16
#SBATCH --time=12:00:00
#SBATCH --output=slurm-refine-16-%j.out
...
singularity exec --cleanenv \
  ...
  bash -lc "
    ...
    echo 'Running Loop 16 Refinement...'
    \$PY_EXE scripts/sample_refined.py \\
      --diff-config configs/ala2_al_16_hpc.yaml \\
      --diff-ckpt runs/day10_al_16_hpc/members/m000/checkpoints/iter_20.pt \\
      --force-ckpt runs/energy_pairwise/best_model.pt \\
      --out-dir runs/loop_b_refinement_16 \\
      --n-samples 10000 \\
      --batch-size 1000 \\
      --warmup-steps 1000 \\
      --keep-percent 0.01 \\
      --refinement-steps 100000 \\
      --step-size 1e-7
  "
