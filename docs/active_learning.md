...
Before the loop begins, an **Ensemble** of $M$ probabilistic diffusion models (typically $M=4$) is initialized.
...
*   **Logic**: A large pool of candidates (e.g., $N=1000$) is generated to explore the landscape.
*   **Consensus Mechanism**: instead of generating from a single model, the **Mean** of the predicted noise from all ensemble members is used to guide the trajectory: $\bar{\epsilon}\_\theta(x\_t, t) = \frac{1}{M} \sum_{i=1}^M \epsilon_{\theta_i}(x\_t, t)$. This stabilizes generation.
*   **Uncertainty Estimation**: Simultaneously, the **Variance** of the noise predictions is computed: $\mathbb{V}[\epsilon] = \frac{1}{M-1} \sum (\epsilon_i - \bar{\epsilon})^2$. This variance is summed over the diffusion trajectory to produce a single scalar "Uncertainty Score" for each generated structure.

### Step 2: Acquisition (Active Selection)
*   **Strategy**: Maximize Uncertainty. The top $k$ candidates (e.g., $k=200$) with the highest accumulated uncertainty scores are selected.
...
*   **Role**: The Oracle acts as the "Ground Truth". In a real wet-lab setting, this would be an experiment. In a simulation environment, it is a lookup into a massive validation dataset or querying a simulator.
